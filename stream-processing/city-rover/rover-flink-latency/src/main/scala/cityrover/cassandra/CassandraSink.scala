...
final class CassandraSinkWriter(
  ...
  override def write(
    ...
    try
      ...
      case ex: Exception =>
        log.error(
          s"Failed to write telemetry event to Cassandra: $event",
          ex
        )
  ...
  override def flush(endOfInput: Boolean): Unit =
    log.info(
      s"Flushing Cassandra writes, endOfInput=$endOfInput"
    )
  ...
  override def close(): Unit =
    ...
    try
      ...
      case ex: Exception =>
        log.warn(
          "Error while waiting for pending writes",
          ex
        )
  ...
  private def waitForPendingWrites(): Unit =
    pendingFutures.synchronized:
      ...
      if !pendingFutures.isEmpty then
        ...
        log.info(
          s"Waiting for ${pendingFutures.size} pending async writes..."
        )
