 ...
 * ConflictDetector
 *
 * Predicts future conflicts using linear motion extrapolation
 * and distance-based threshold evaluation.
 */
class ConflictDetector(maxGapMs: Long = 5000L) {
  ...
  def detect(
    ...
    println(s"[CONFLICT] action=trajectoriesBuilt count=${trajectories.size}")

    // ------------------------------------------------------------
    // 2. Pairwise prediction
    // ------------------------------------------------------------
    ...
    for {
      ...
      if (relativeSpeed <= 1e-6) {
        println(s"[CONFLICT] skipZeroSpeed pair=($id1,$id2)")
      } else {
        ...
        futurePositions.find(_._2 <= thresholdMeters).foreach {
          case (t, d) =>
            ...
            println(s"[CONFLICT] detected pair=($id1,$id2) t=$t distance=$d")
        }
      }
    }
    ...
    println(
      s"[CONFLICT] summary events=${events.size} conflicts=${interactions.size} timeMs=$elapsedMs"
    )
  ...
  private def computeVelocity(traj: Trajectory): (Double, Double) = {
    ...
    val dx = (lon2 - lon1) * metersPerLon
    val dy = (lat2 - lat1) * metersPerLat
