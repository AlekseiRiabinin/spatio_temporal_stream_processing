...
# 1. Start Postgis
# ------------------------------------------------------------
echo "1. Starting Postgis..."
docker compose -f "$COMPOSE_FILE" up -d postgis-real-deal

# ------------------------------------------------------------
# 2. Start Kafka first (required for rover-simulator)
# ------------------------------------------------------------
echo "2. Starting Kafka (Single-Node KRaft)..."
docker compose -f "$COMPOSE_FILE" up -d kafka-1

echo ""
echo "3. Waiting for Kafka to become ready..."
...
# ------------------------------------------------------------
echo ""
echo "5. Starting graph-engine and rover-simulator..."
